...
describe("POST /stats/grouped/public", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("should return stats per group without identity headers", async () => {
    mockExecute.mockResolvedValueOnce({
      rows: [makeStatsRow({ emailsSent: 800, emailsReplied: 3, recipients: 400 })],
    });
    mockExecute.mockResolvedValueOnce({
      rows: [makeStatsRow({ emailsSent: 200, emailsReplied: 1, recipients: 100 })],
    });
  ...
  it("should return 500 on db error", async () => {
    ...
    const response = await request(app)
      .post("/stats/grouped/public")
      .send({ groups: { "failing-group": { runIds: ["run-1"] } } });
