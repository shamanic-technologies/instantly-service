...
- **Key resolution:** reads the shared cold-email workspace key from `process.env.INSTANTLY_API_KEY` (key-service is org-scoped, unreachable for a platform read). **Requires `INSTANTLY_API_KEY` on the Railway env** (shared var, same key the audit/heal/cleanup CLIs use); unset → fail loud 500, no impact on other routes. Fail-loud throughout — no silent zero fallbacks.
