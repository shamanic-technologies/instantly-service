...
- **Known limitation — step-level stats stay raw event counts.** Manual qualification events carry no `step` (step = NULL) and are excluded by `e.step IS NOT NULL`, so the per-step `stepStats` breakdown does NOT reflect manual reclassifications (the overall/grouped totals do). A reply re-qualified after the fact is correct in the model/brand totals but not in the per-step drilldown.
