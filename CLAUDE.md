...
- **Metrics + source of truth — Instantly is SoT for ALL five** (it runs the campaign, sends the email, stores the contact, holds the sequence). `activeCampaigns` / `emailsSent` / `contactedDispatched` / `contactsStored` sum directly from `/campaigns/analytics`. `pendingSends` (remaining un-sent steps = the forecast volume magnitude) is **DERIVED**: Instantly has no ready-made remaining-step field, but each campaign's `sequences` carry the loaded followup steps and analytics carry `emails_sent_count`, so (one campaign = one lead) Instantly's remaining = `Σ over ACTIVE campaigns of max(0, stepCount − sent)` (`computeInstantlyPendingSends`). Do NOT re-mark `pendingSends` local-only — the data is derivable, not missing (regression guard: it was shipped local-only in v0.50.4, corrected in the next hotfix).
...
- **Local side = one round-trip.** Five scalar subqueries in a single `db.execute`. `pendingSends` reuses the EXACT `loadPendingLeads` gate so its number equals the total steps the forecast projects. Local + both Instantly sweeps fetched via `Promise.all`; any failing → **fail loud 500**, no fabricated zero.
