...
- `src/db/schema.ts` — Drizzle ORM table definitions (campaigns, leads, accounts, events). Gold stats are derived-on-read from silver in `analytics.ts`/`status.ts` — there is NO `analytics_snapshots` materialized table (a dropped legacy table; do not reintroduce a snapshot **table** cache). Live aggregation was proven too slow under load (2026-06-13: `/orgs/stats` + `/public/stats` flooded email-gateway with ~10s AbortSignal timeouts — bursts of identical re-aggregations saturating the 0.25-1 CU Neon compute). The mitigation is the LIGHTEST possible, NOT a snapshot table: (1) a short-TTL in-memory cache (`src/lib/stats-cache.ts`, 60s default, per-replica) on both GET `/stats` and GET `/public/stats` — keyed on params (+ orgId for the authed route); (2) `instantly_events_stats_covering_idx` (campaign_id, event_type, lead_email, step) → index-only scan on the filtered `/orgs/stats` path (does NOT help the no-filter `/public/stats`, which the planner seq-scans regardless — that path leans on the cache); (3) `queryStats` / `queryGroupedStats` / the per-step block (`computeStepStats`/`computeStatsPayload`) parallelize their independent reads via `Promise.all`. If you add a new cached stats route, `clearStatsCache()` MUST be called in its unit-test `beforeEach` or cross-test cache hits skip the DB and break assertions.
