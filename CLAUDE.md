...
- **Kill-switch env `STOP_ON_CLICK_ON_SIGNUP_ENABLED`, default OFF** (exactly `"true"` = ON, else OFF — mirrors `DELETE_FINISHED_CONTACTS_ENABLED`). Ships dormant. Arming requires `BRAND_SERVICE_URL` + `BRAND_SERVICE_API_KEY` set on Railway AND the flag flipped to `"true"`.
