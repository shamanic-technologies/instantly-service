...
**Never embed the parameterized `localDayKey` fragment in both SELECT and GROUP BY of the SAME query — group by ordinal (`GROUP BY 1`) or a CTE alias instead.** For `groupBy=day`, `groupCol` is `localDayKey(...)`, which carries the `${timezone}` bind. drizzle re-emits that bind on EVERY interpolation of the fragment, so the timezone lands under `$1` in the SELECT but a different `$N` in the GROUP BY. Postgres matches grouped columns by exact expression (param position included), so the mismatched placeholders trip `42803` (`column "c.created_at" must appear in the GROUP BY clause`). Non-day groupings never hit this — their `groupCol` is `sql.raw(col)` (param-free, byte-identical every interpolation). The sibling day queries dodge it via a CTE that names the expression once (`${groupCol} AS group_key` → `GROUP BY group_key` — see `queryGroupedEvents`/sentiment); `queryGroupedCampaignAggregates` uses `GROUP BY 1` (no CTE). Historic bug (v0.46.2): the aggregates query inlined `${groupCol}` in SELECT+WHERE+GROUP BY → every `/orgs/stats?groupBy=day` 500'd → features-service pipeline-activity 502'd. Unit tests mock `db.execute` so they CANNOT catch param-drift — the guard is the static-SQL assertion `aggregateSqlText.not.toContain("GROUP BY TO_CHAR")` in the day tests. Any NEW grouped day query needs the same assertion.
