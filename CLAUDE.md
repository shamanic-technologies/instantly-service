...
- **Reconciles COUNTS, never the forecast's future dates.** The sending-forecast VOLUME is an inherent projection — the exact future send DATE of a step exists nowhere (Instantly schedules dynamically off daily limits / business hours / throttling / pauses), so timing cannot be reconciled. Do NOT add a "date reconciliation" — it would compare against a number that doesn't exist.
- **Metrics + source of truth.** `activeCampaigns` / `emailsSent` / `contactedDispatched` / `contactsStored` all have a true Instantly counterpart (Instantly runs the campaign, sends the email, stores the contact against plan quota) → Instantly is SoT, we return its exact number + the local drift as `delta`. `pendingSends` (the forecast volume magnitude, = provisioned `sequence_costs` steps on live campaigns) is **local-only** (`instantly:null`, `delta:null`, `sourceOfTruth:'local'`): Instantly exposes no remaining-step count, so there is no honest counterpart. Do NOT fabricate an Instantly `pendingSends`.
- **Instantly side = ONE bulk call.** `listAllCampaignAnalytics` → `GET /campaigns/analytics` with `id` omitted returns ALL campaigns' analytics as a single **non-paginated** array (confirmed via developer.instantly.ai — no cursor/limit on this endpoint), so it is the ONE Instantly-client helper that deliberately does NOT paginate. Per-campaign iteration is infeasible at fleet scale (thousands of campaigns × rate limit). Instantly ACTIVE lifecycle code = `1`.
- **Local side = one round-trip.** Five scalar subqueries in a single `db.execute`. `pendingSends` reuses the EXACT `loadPendingLeads` gate so its number equals the total steps the forecast projects. Local + Instantly fetched via `Promise.all`; either failing → **fail loud 500**, no fabricated zero.
